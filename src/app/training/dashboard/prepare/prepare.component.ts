...
export class PrepareComponent implements OnInit {
    ...
    selected: string;
    constructor(private render: Renderer2, private route: Router, private userS: UserService) {}
    ...
    wasClicked(event, cir1, cir2, cir3) {
        ...
        if (this.selected !== 'Level 1') {
            ...
            arr.forEach((value) => {
                value === event.target.parentNode ? this.render.addClass(value, 'whiten') : this.render.removeClass(value, 'whiten');
            });
        }
    }
    startProgram(event) {
