...
export class BmiComponent implements OnInit {
    ...
    getBMI() {
        ...
        this.bmiIndex = +(weight / height / height * 10000 + (age > 30 ? 0.1 : -1.1)).toFixed(2)
    ...
    getBMIinfo(bmi: number): string {
        ...
        if (bmi < 25) return 'Idealna tezina samo nastavi tako uz redovan trening i dobru kasiku'
        if (bmi < 28) return 'Ako si bilder onda je uredu , ako nisi ili smanji kasiku ili uzmi tegove '
        if (bmi < 30) return 'Prepurucujem suplju kasiku i vise kretanja (debeo si)'
    }
}
